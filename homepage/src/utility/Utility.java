...
public class Utility {
    ...
    public static List<String> getDay(){
        ...
        SimpleDateFormat sd = new SimpleDateFormat("yyyy-MM-dd"); //SimpleDateFormat 데이터포멧 설정
        Calendar cal = Calendar.getInstance(); //현재날짜
        for(int j = 0; j < 3; j++){
            list.add(sd.format(cal.getTime())); //현재날짜를 리턴한다.
            cal.add(Calendar.DATE, -1); //오늘날짜에서 하루전 날을 뽑아낸다.
        }
...
//	    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	  ...
	  public static String paging(int totalRecord, int nowPage, int recordPerPage, String col, String word){ 
	    int pagePerBlock = 10; // 블럭당 페이지 수 
	    int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage));// 전체 페이지  Math.ceil는 반올림을 하여 마지막 페이지를 보여주게 해준다.
	    int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
	    int nowGrp = (int)(Math.ceil((double)nowPage/pagePerBlock));    // 현재 그룹 
	    int startPage = ((nowGrp - 1) * pagePerBlock) + 1; // 특정 그룹의 페이지 목록 시작  
	    int endPage = (nowGrp * pagePerBlock);             // 특정 그룹의 페이지 목록 종료   
